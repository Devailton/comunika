...
/*		$("#slider .ls-slide").attr('data-ls','transition2d:5;slidedelay:5000;');	
	...
	/* Check box customiza */
	$('#cadastroForm').find('input').each(function(){
		var self = $(this),
		label = self.next(),
...
function twitter_carousel_init (){
	...
	/* Lista de medicos, trocando as cores*/
...
function init_map()
{
	var coordLat = -20.754375;
	var coordLng = -42.879363;
...
{	
	...
	/* publicidade */
	/**/
	$('#publicidade .grid').isotope({
		masonry: {columnWidth: $('#publicidade .item').outerWidth()}
	});
...
$(function()
{
	...
	$('#publicidade .grid').isotope({
		itemSelector : '.item',
		masonry: {columnWidth: $('#publicidade .item').eq(0).outerWidth()}
	});
	$('#publicidade .filter').on('change', function()
	{
		$('#publicidade .grid').isotope(
	 	{
	 		filter: $(this).val()
	...
	$('#wellness-doctors-carousel').owlCarousel({
  	...
  	/**/
	/* wellness publicidade carousel */
	/**/
	$('#wellness-publicidade-carousel').owlCarousel({
		items: 3,
		itemsDesktop: false,
		itemsDesktopSmall: false,
		itemsTablet: [980,2],
